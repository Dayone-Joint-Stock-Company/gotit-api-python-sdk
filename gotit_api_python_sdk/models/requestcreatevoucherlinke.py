...
class REQUESTCREATEVOUCHERLINKE(BaseModel):
    """
    ...
    amount: StrictInt = Field(description="+ Voucher value. Ensures the following conditions: + Is a multiple of 1000 + Is within the price range minimum - maximum value that the partner and Got It have agreed upon.")
    ...
    product_id: Optional[StrictInt] = Field(default=None, description="+ Product code. Specify the template displayed on the voucher (list of Categories and Brands). + If no transmission will use Got It's default productId", alias="productId")
    use_otp: StrictInt = Field(description="= 1 if the end user is required to enter a password to view the voucher. Example: 1")
    ...
    signature: StrictStr = Field(description="RSA encryption, used for client authentication")
    __properties: ClassVar[List[str]] = ["orderName", "transactionRefId", "amount", "expiryDate", "productId", "use_otp", "otp_type", "password", "receiver_name", "phone", "email", "signature"]
